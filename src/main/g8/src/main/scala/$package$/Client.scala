...
object Client extends App {
  ...
  val statusRequestIO =
    statusRequest.run(esiClient).map(_.right.get.asJson.spaces2)
  ...
  val statsRequest = for {
    ...
    kills <- universe
              ...
              .run(esiClient)
              .map(_.right.get.sortBy(0 - _.npc_kills).take(10))
    // and and let's look up their map data
    results <- kills.map { stats =>
                for {
                  system <- universe
                             .getUniverseSystemsSystemId(stats.system_id)
                             .run(esiClient)
                             .map(_.right.get)
                  constellation <- universe
                                    .getUniverseConstellationsConstellationId(system.constellation_id)
                                    .run(esiClient)
                                    .map(_.right.get)
                  region <- universe
                             .getUniverseRegionsRegionId(constellation.region_id)
                             .run(esiClient)
                             .map(_.right.get)
  ...
  val statsText = statsRequest.unsafeRunSync
    .map {
      case (stats, system, constellation, region) =>
